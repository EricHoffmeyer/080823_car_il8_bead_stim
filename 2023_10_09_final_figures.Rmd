...
The DESeq gene list is already sorted by adjusted p value, so we can start there. I'm boosting the gene level to 26 since we artificially increased CXCR2 expression.
...
no_anno <- data.frame(sample_name = colnames(no_matrix_log2)) |> 
  mutate(CAR = if_else(sample_name %in% no_b7c2_samples, "B7H3-CXCR2", "B7H3")) |> 
  column_to_rownames("sample_name")
...
r <- pheatmap(no_matrix_log2,
         ...
         annotation = no_anno,
         annotation_colors = anno_colors,
         main = "Top 25 genes by log2FC value: no stim")
ggsave(filename = paste0(folder.name, subfolder.name, "/BC_vs_B7_no_stim_condition_heatmap_top25_log2FC.png"),
       plot = r,
       width = 7,
       height = 9,
       units = "in")
...
This requires a new subsetting of the original bulkRNAseq, since all the previous conditions were BC vs B7 for a single condition, not across conditions.
...
Subset data to BC CARs, and remove the bead condition
...
There is only 1 differentially expressed gene, so I'm stopping here with this anaylsis.
